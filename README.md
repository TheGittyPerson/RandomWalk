...
Each step in the walk is generated by **`randomwalk/rw_generator.py`** by randomly choosing:
- an x-direction and x-distance
- a y-direction and y-distance
from the **lists defined in `RandomWalk.walk` attributes**.

`randomwalk/random_walk.py` creates an instance of the generator and plots the values on a graph.
...
The example code lives in `randomwalk/__main__.py`.
...
See the documentation within `randomwalk/rw_graph_properties.py` and `randomwalk/rw_settings.py` for more customizable attributes.
...
- `randomwalk/random_walk.py` — main module; contains the `RandomWalk` class
- `randomwalk/rw_generator.py` — controls movement logic (generates plot coordinates)
- `randomwalk/rw_graph_properties.py` — controls Matplotlib graph settings; imported from the main module
- `randomwalk/rw_settings.py` — controls random walk behavior settings; imported from the main module to be passed to the `RWGenerator` instance
- `randomwalk/__main__.py` — example usage, runnable with `python -m randomwalk`
